...
//Function compares user's guess to secret word
...
void checkGuess(char guess[6], char secretWord[6]){
    ...
    //If guess is incorrect, check each character in guess & print in appropriate color
        ...
        for(int i = 0; guess[i] != '\0'; ++i){
            ...
            //Printf in red if not found
            //FIXME
            //Only prints yellow characters from guess that are in secretWord
            //Needs to print red characters from guess that are not in secretWord
            else{
...
int main(){
    //Secret word will be ranodmly selected by another function
